...
updated = "2026-02-23T18:21:51.765370Z"
tags = []
links = []
+++
...
Implemented core Specs surface improvements: dedicated specs nav section in sidebar, specs list page with cleaner library-style rows, and full-screen detail editor panel using MarkdownEditor split mode. Tauri spec CRUD commands are wired and validated via build + cargo check.

Editor ergonomics pass: create dialogs now use larger viewport-constrained surfaces and default to edit mode (split still available) to avoid cramped split-pane authoring. Detail editors now render centered in wider sheets for full-document work.
