...
import { Issue, IssueEntry, StatusConfig, getStatusStyles } from '../types';
...
function normalizeIssue(issue: Issue): Issue {
  const metadata = (issue as Partial<Issue>)?.metadata as Partial<Issue['metadata']> | undefined;
  return {
    metadata: {
      ...
      links: Array.isArray(metadata?.links)
        ? metadata.links.filter((link): link is string => typeof link === 'string')
        : [],
    },
    description: typeof (issue as Partial<Issue>)?.description === 'string'
      ? issue.description
      : '',
  };
}

function parseLink(raw: string): { type: LinkType; target: string } {
  const idx = raw.indexOf(':');
  if (idx > 0) {
    const maybeType = raw.slice(0, idx) as LinkType;
    const target = raw.slice(idx + 1);
    if (LINK_TYPES.includes(maybeType) && target.trim()) {
      return { type: maybeType, target };
    }
  }
...
export default function IssueDetail({
  ...
  const createdDate = (() => {
    const date = new Date(draft.metadata.created);
  ...
  const displayTitle = draft.metadata.title.trim() || deriveTitleFromFileName(entry.file_name);

  const assigneeInitials = useMemo(() => {
    const name = (draft.metadata.assignee ?? '').trim();
    if (!name) return '';
    const parts = name.split(/\s+/).filter(Boolean);
    return parts.slice(0, 2).map((p) => p[0]?.toUpperCase() ?? '').join('');
  }, [draft.metadata.assignee]);

  const linkRows = draft.metadata.links.map(parseLink);

  const filteredSuggestions = tagSuggestions
    .filter((tag) => !draft.metadata.tags.includes(tag))
  ...
  const removeTag = (tag: string) => {
    setDraft((current) => ({
      ...
      metadata: {
        ...current.metadata,
        tags: current.metadata.tags.filter((t) => t !== tag),
      },
    }));
    markDirty();
  };
  ...
  const addLink = () => {
    ...
    if (!target) return;
    const serialized = `${newLinkType}:${target}`;
    setDraft((current) => ({
      ...current,
      metadata: {
        ...current.metadata,
        links: [...current.metadata.links, serialized],
      },
    }));
    setNewLinkTarget('');
    setNewLinkType('relates-to');
  ...
  const removeLink = (index: number) => {
    setDraft((current) => ({
      ...
      metadata: {
        ...current.metadata,
        links: current.metadata.links.filter((_, i) => i !== index),
      },
    }));
    markDirty();
  };
