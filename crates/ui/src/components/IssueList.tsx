...
import { IssueEntry, StatusConfig, getStatusStyles } from '../types';
...
function deriveTitle(entry: IssueEntry): string {
  const providedTitle = entry.issue?.metadata?.title;
...
function IssueCard({ entry, isMoving, onSelect }: IssueCardProps) {
  ...
  const created = typeof entry.issue?.metadata?.created === 'string' ? entry.issue.metadata.created : '';
  const tags = Array.isArray(entry.issue?.metadata?.tags)
    ? entry.issue.metadata.tags.filter(Boolean).slice(0, 2)
    : [];
