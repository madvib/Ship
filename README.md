...
At each transition, Shipwright knows where you are and what your agents need to know. There are also Notes and ADR modules to provide long-term memory outside the workflow loop.
...
| ------------ | ------------ | --------------------------- | ----------------------- |
| Claude Code  | `CLAUDE.md`  | `.mcp.json` (JSON)          | `.claude/skills/*.md` |
| Gemini CLI   | `GEMINI.md`  | `.gemini/settings.json`     | `.gemini/skills/*.md`   |
| OpenAI Codex | `AGENTS.md`  | `.codex/config.toml` (TOML) | `.agents/skills/*.md`   |
