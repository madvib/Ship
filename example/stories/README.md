...
# Run any story
bash example/stories/solo-dev/story.sh
bash example/stories/multi-provider/story.sh
bash example/stories/team-handoff/story.sh
```

Pass `--skip-build` to skip the cargo build step if the binary is already fresh:

```bash
bash example/stories/solo-dev/story.sh --skip-build
```
